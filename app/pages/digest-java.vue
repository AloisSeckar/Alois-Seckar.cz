  ...
  <div>
    ...
    <div id="content" class="content-pane">
      <p class="text-left sm:text-justify my-4">
        Sbírka zajímavých článků a&nbsp;aktuálních novinek ze světa Javy spravovaná
        v&nbsp;repozitáři
        <NuxtLink to="https://github.com/AloisSeckar/demos-java" class="inline-block">
...
function fetchArticles(offset: number) {
  return $fetch<ArticleItem[]>('/get-articles', {
    method: 'POST',
    body: { url: JAVA_NEWS_URL, count: PAGE_SIZE, offset },
  })
}

// client-only fetch
const { data, status } = useAsyncData('java-news-feed', () => fetchArticles(0), { server: false, lazy: true })

...
export function getAVGSpeed(time: string, length: number): string {
  ...
  if (timeChunks.length === 4) { // time contains hours...
    if (parseInt(timeChunks[3] || '0') >= 5) { // round miliseconds
      timeInSeconds = 1
    }
    timeInSeconds += parseInt(timeChunks[2] || '0')
    timeInSeconds += parseInt(timeChunks[1] || '0') * 60
    timeInSeconds += parseInt(timeChunks[0] || '0') * 3600
  } else { // time only consists from mins and secs...
    if (parseInt(timeChunks[2] || '0') >= 5) { // round miliseconds
      timeInSeconds = 1
    }
    timeInSeconds += parseInt(timeChunks[1] || '0')
    timeInSeconds += parseInt(timeChunks[0] || '0') * 60
  }
...
export function sumTimes(time1: string, time2: string): string {
  ...
  if (timeChunks_1.length < 4) { // normalize to 4 pcs format if needed
    timeChunks_1[3] = timeChunks_1[2] || 0
    timeChunks_1[2] = timeChunks_1[1] || 0
    timeChunks_1[1] = timeChunks_1[0] || 0
    timeChunks_1[0] = 0
  }
  ...
  if (timeChunks_2.length < 4) { // normalize to 4 pcs format if needed
    timeChunks_2[3] = timeChunks_2[2] || 0
    timeChunks_2[2] = timeChunks_2[1] || 0
    timeChunks_2[1] = timeChunks_2[0] || 0
    timeChunks_2[0] = 0
  }
  // count together
  ...
  let temp = 0
  // miliseconds
  output[3] = (timeChunks_1[3] || 0) + (timeChunks_2[3] || 0)
  if (output[3] > 9) {
    temp = 1
  }
  output[3] = output[3] % 10
  // seconds
  output[2] = (timeChunks_1[2] || 0) + (timeChunks_2[2] || 0) + temp
  temp = Math.floor(output[2] / 60)
  output[2] = output[2] % 60
  // minutes
  output[1] = (timeChunks_1[1] || 0) + (timeChunks_2[1] || 0) + temp
  temp = Math.floor(output[1] / 60)
  output[1] = output[1] % 60
  // hours
  output[0] = (timeChunks_1[0] || 0) + (timeChunks_2[0] || 0) + temp
  //
